...
function Inner_binary(;m1 = 1., m2 = 1., a = 1000., e_mag = 0.3)
    ...
    inbn = Inner_binary(m1, m2, a_hat, b_hat, c_hat, S1, S2, E, a, e_mag, L_mag)
    return inbn
end
...
function Inner_binary!(inbn::Inner_binary, outsd::Third_wheel, M0::Float64)
    ...
    e_vec = e_mag*a_hat
    𝚥_vec = sqrt(1-e_mag^2)*c_hat
    δe_vec = cross( dot(𝚥_vec, A_hat)*A_hat + dot(𝚥_vec, B_hat)*B_hat, e_vec ) -4*cross(𝚥, e_vec) -5*cross(  dot(e_vec, A_hat)*A_hat + dot(e_vec, B_hat)*B_hat, 𝚥_vec )
    δe_vec = 0.5*π*(a/q)^1.5*m_3/sqrt(2*M12*M123)*δe_vec
    δ𝚥_vec = cross( dot(𝚥_vec, A_hat)*A_hat + dot(𝚥_vec, B_hat)*B_hat, 𝚥_vec ) -5*cross(  dot(e_vec, A_hat)*A_hat + dot(e_vec, B_hat)*B_hat, e_vec )
    ...
    e_mag = norm(e_vec)
    L_mag = norm(L_vec)
    δL_vec = L_vec - inbn.L_mag*inbn.c_hat

    inbn = Inner_binary(inbn, a_hat, b_hat, c_hat, S1, S2, E, a, e_mag, L_mag)
    return inbn, δL_vec
end
...
function Third_wheel!(outsd::Third_wheel, inbn::Inner_binary, δL_vec::Array{Float64})
    L3_vec = outsd.L3_vec - δL_vec

    𝚥_vec = sqrt(1-inbn.e_mag^2)*inbn.c_hat
    𝚥_A   = dot(𝚥, outsd.A_hat)
    𝚥_B   = dot(𝚥, outsd.A_hat)
    e_vec = inbn.e_mag*inbn.a_hat
